 ...
 * @subpackage unblockadblocker/public
 */
...
class unblockadblocker_Public {
    ...
    public function localize_script() {
        ...
        $plugin_data = [];
        $exclude = ['unblockadblocker-file-name', 'unblockadblocker-version'];
        foreach ( $this->settings as $key => $value) {
            if( in_array( $key, $exclude ) ) {
                continue;
            }
            $plugin_data[str_replace( 'unblockadblocker-', '', $key )] = $value;
        }
    ...
    public function ajax_request_callback() {
        ...
        $ip = $this->getUserIP();
        if($ip != "::1"){
            $api = "https://ip-api.io/json/".$ip;
            $response = wp_remote_get($api);
            if ( ( !is_wp_error($response)) && (200 === wp_remote_retrieve_response_code( $response ) ) ) {
                $responseBody = json_decode($response['body']);
                if(isset($responseBody->country_name)){
                $country = sanitize_title($responseBody->country_name);  
                }
                else {
                    $country = 'all_country';
                }
        ...
        $final_array = array();
        foreach($this->settings as $key=>$value){
            $key = str_replace($this->plugin_name.'-','',$key);
            $key = str_replace('-all_country','',$key);
    ...
    public function get_options_data() {
        ...
        if( ! empty($settings ) ) {
            if(isset($settings[$this->plugin_name . '-content-'.$this->country])){
            $settings[$this->plugin_name . '-content'] = wpautop($settings[$this->plugin_name . '-content-'.$this->country] );
            unset($settings[$this->plugin_name . '-content-'.$this->country]);
            }
        }
        unset($settings[$this->plugin_name . '-content-']);
        return $settings;
    ...
    public function regenerate_files() {
        ...
        if( empty( $file_name ) ) {
            $file_name = unblockadblocker::get_random_string();
        ...
        $css_file = plugin_dir_path( __DIR__ ) . '/public/css/unblockadblocker-public.css';
        $js_file  = plugin_dir_path( __DIR__ ) . '/public/js/unblockadblocker-public.js';
        ...
        if( ! file_exists( $new_css_file ) || ! file_exists( $new_js_file ) ) {
            try {
                ...
                $contents = '';
                $string_to_replace = "ua-unblockadblocker";
    ...
    public function check_version() {
        ...
        if( $this->settings[$this->plugin_name . '-version'] != UADB_PLUGIN_VERSION ) {
    ...
    public function delete_old_files() {
        ...
        $this->settings[$this->plugin_name . '-version'] = UADB_PLUGIN_VERSION;
    ...
    public function sync_settings() {
        $defaults = unblockadblocker::get_defaults($this->country);
